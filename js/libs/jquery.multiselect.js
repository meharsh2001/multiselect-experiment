...
!(function (t) {
  ...
  function parse2px(dimText, $elem, isHeight) {
    ...
    switch (unit) {
      ...
      case "mm":
        pixels =
          { pt: 4.0 / 3.0, in: 96.0, cm: 96.0 / 2.54, mm: 96.0 / 25.4 }[unit] *
          value;
        break;
      case "em":
  ...
  $.widget("ech.multiselect", {
      ...
      if (this.options.header.constructor == Object) {
        ...
        for (var x = 0; x < options.length; x++) {
          ...
          if (
            ...
            headerLinksHTML += this._linkHTML(
              '<li><a class="{{class}}" title="{{title}}">{{icon}}<span>' +
                displayText +
                "</span></a></li>",
              linkInfoKey
            );
    ...
    _makeOption: function (option) {
      ...
      var inputID =
        ...
        this.multiselectID +
        "-" +
        (option.id || id + "-option-" + this.inputIdCounter++);
          ...
          if (
            ...
            (!!this.$button
              ? target !== this.$button[0] &&
                !$.contains(this.$button[0], target)
              : true) &&
            target !== this.$menu[0] &&
            !$.contains(this.$menu[0], target)
          ) {
            this.close();
          }
        }.bind(this)
    ...
    _setMenuHeight: function (recalc) {
      ...
      if (/\d/.test(optionHeight)) {
        ...
        var menuBorderPaddingHt =
          this.$menu.outerHeight(false) - this.$menu.height();
        var cbBorderPaddingHt =
          this.$checkboxes.outerHeight(false) - this.$checkboxes.height();

        optionHeight = parse2px(optionHeight, self.element, true).px;
        maxHeight =
          Math.min(optionHeight, maxHeight) -
          headerHeight -
          menuBorderPaddingHt -
          cbBorderPaddingHt;
    ...
    _traverse: function (which, start) {
      ...
      if (!$next.length) {
        ...
        $container
          ...
          .trigger("mouseover", { allowScroll: true });
    ...
    flipAll: function (groupID) {
      ...
      var $filteredOptgroupInputs = gotID
        ? this._multiselectOptgroupFilter(groupID).find("input")
        : null;
      var gInputCount = gotID ? $filteredOptgroupInputs.length : 0;
      var gCheckedCount = gotID
